					...
					<table class="table">
						<thead>
							<tr>
								...
								<th>数量</th>
								<th>添加日期</th>
								<th  colspan="2" style="width: 90px;text-align:center">编辑</th>
							</tr>
						</thead>
						<tbody>
							<c:forEach items="${list}" var="item">
								<tr>
									...
									<td>${item.cprice}</td>
									<td>
									    <c:if test="${item.pic==null}">无</c:if>
										<c:if test="${item.pic!=null}"><img src="<%=basePath%>upload/${item.pic}" style="width:50px;height:50px;"/></c:if>
									</td>
									<td style="width:200px;">${item.pdesc}</td>
									<td>
